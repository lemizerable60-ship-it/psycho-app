...
const ApiKeyModal = ({ onSave }) => {
    ...
    return React.createElement('div', { className: 'modal-overlay' },
        React.createElement('div', { className: 'card', style: { maxWidth: '500px' } },
            React.createElement('h2', null, 'Введите ваш Gemini API ключ'),
            React.createElement('p', null, 'Для работы функции анализа результатов нужен ваш личный ключ API. Получить его можно в ', React.createElement('a', { href: 'https://aistudio.google.com/app/apikey', target: '_blank' }, 'Google AI Studio'), '.'),
            React.createElement('p', null, 'Ключ будет сохранен локально в вашем браузере.'),
            ...
            React.createElement('button', { onClick: () => onSave(key), disabled: !key.trim(), style: { marginTop: '10px' } }, 'Сохранить и продолжить')
        )
    );
};
...
const ClientForm = ({ client, onSave, onCancel }) => {
  ...
  return React.createElement('form', { onSubmit: handleSubmit, className: 'card' },
    React.createElement('h2', null, client ? 'Редактировать клиента' : 'Новый клиент'),
    React.createElement('label', null, 'ФИО',
      React.createElement('input', { name: 'fullName', value: formData.fullName, onChange: handleChange, required: true })
    ),
    React.createElement('label', null, 'Дата рождения',
      React.createElement('input', { type: 'date', name: 'birthDate', value: formData.birthDate, onChange: handleChange })
    ),
    React.createElement('label', null, 'Заметки',
      React.createElement('textarea', { name: 'notes', value: formData.notes, onChange: handleChange, rows: 4 })
    ),
    React.createElement('div', { style: { display: 'flex', gap: '10px', marginTop: '10px' } },
      React.createElement('button', { type: 'submit' }, 'Сохранить'),
      React.createElement('button', { type: 'button', className: 'text-button', onClick: onCancel }, 'Отмена')
    )
  );
};

const ClientListScreen = ({ clients, onSelectClient, onAddClient }) => React.createElement('div', null,
  React.createElement('div', { className: 'header' },
    React.createElement('h1', null, 'Клиенты'),
    React.createElement('button', { onClick: onAddClient }, '+ Добавить клиента')
  ),
  React.createElement('div', { className: 'card' },
    clients.length > 0 ? clients.map(client =>
      React.createElement('div', { key: client.id, className: 'client-list-item', onClick: () => onSelectClient(client) },
        React.createElement('h3', null, client.fullName),
        React.createElement('p', { style: { color: 'var(--secondary-text-color)', margin: 0 } }, client.birthDate)
      )
    ) : React.createElement('p', null, 'Список клиентов пуст. Нажмите "+ Добавить клиента", чтобы начать.')
  )
);

const ClientDetailScreen = ({ client, results, onBack, onStartTest, onSelectResult }) => React.createElement('div', null,
  React.createElement('div', { className: 'header' },
    React.createElement('button', { className: 'text-button', onClick: onBack }, '← К списку клиентов')
  ),
  React.createElement('div', { className: 'card' },
    React.createElement('h2', null, client.fullName),
    React.createElement('p', null, React.createElement('strong', null, 'Дата рождения:'), ` ${client.birthDate}`),
    React.createElement('p', null, React.createElement('strong', null, 'Заметки:'), ` ${client.notes || 'Нет'}`)
  ),
  React.createElement('div', { className: 'card' },
    React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' } },
      React.createElement('h3', null, 'История диагностик'),
      React.createElement('button', { className: 'secondary', onClick: onStartTest }, '+ Новая диагностика')
    ),
    results.length > 0 ? results.map(result =>
      React.createElement('div', { key: result.id, className: 'test-history-item', onClick: () => onSelectResult(result) },
        React.createElement('strong', null, result.testName),
        React.createElement('p', { style: { color: 'var(--secondary-text-color)', margin: 0 } }, new Date(result.date).toLocaleDateString())
      )
    ) : React.createElement('p', null, 'Еще не было проведено ни одной диагностики.')
  )
);

const TestSelectionScreen = ({ onSelectTest, onBack }) => React.createElement('div', null,
  React.createElement('div', { className: 'header' },
    React.createElement('button', { className: 'text-button', onClick: onBack }, '← Назад к клиенту')
  ),
  React.createElement('div', { className: 'card' },
    React.createElement('h2', null, 'Выберите методику'),
    TESTS.map(test =>
      React.createElement('div', { key: test.id, className: 'client-list-item', onClick: () => onSelectTest(test) },
        React.createElement('h3', null, test.name),
        React.createElement('p', { style: { color: 'var(--secondary-text-color)', margin: 0 } }, test.description)
      )
    )
  )
);
...
const TestRunnerScreen = ({ client, test, onFinish, onBack }) => {
    const [answers, setAnswers] = useState(new Array(test.questions.length).fill(null));
    ...
    const handleSubmit = () => {
        const { score } = test.getScore(answers);
    ...
    return React.createElement('div', null,
        React.createElement('div', { className: 'header' },
            React.createElement('button', { className: 'text-button', onClick: onBack }, '← Отмена')
        ),
        React.createElement('div', { className: 'card' },
            React.createElement('h2', null, test.name),
            React.createElement('p', null, `Клиент: ${client.fullName}`),
            React.createElement('hr', { style: { margin: '20px 0', border: 'none', borderBottom: '1px solid var(--divider-color)' } }),
            test.questions.map((q, i) =>
                React.createElement('div', { key: i, className: 'question-group' },
                    React.createElement('p', null, React.createElement('strong', null, `${i + 1}. ${q.text}`)),
                    React.createElement('div', { className: 'radio-group' },
                        q.options.map((opt, optIndex) => {
                            ...
                            return React.createElement('label', { key: optIndex },
                                React.createElement('input', {
                                    type: 'radio',
                                    name: `q-${i}`,
            ...
            React.createElement('button', { onClick: handleSubmit, disabled: answers.some(a => a === null) }, 'Завершить и рассчитать')
        )
    );
};
...
Твоя задача - предоставить профессиональную интерпретацию этого результата.`;
    ...
    return React.createElement('div', null,
        React.createElement('div', { className: 'header' },
            React.createElement('button', { className: 'text-button', onClick: onBack }, '← Назад к клиенту'),
            React.createElement('button', { onClick: handleExportTXT, disabled: isLoading || !result.interpretation }, 'Экспорт в TXT')
        ),
        React.createElement('div', { className: 'card' },
            React.createElement('h2', null, `Результаты: ${result.testName}`),
            React.createElement('p', null, React.createElement('strong', null, 'Клиент:'), ` ${client.fullName}`),
            React.createElement('p', null, React.createElement('strong', null, 'Дата:'), ` ${new Date(result.date).toLocaleString()}`),
            React.createElement('p', null, React.createElement('strong', null, 'Итоговый балл:'), ` ${result.score}`),
            React.createElement('p', null, React.createElement('strong', null, 'Заключение по шкале:'), ` ${summary}`),
            React.createElement('h3', null, 'Интерпретация (сгенерировано ИИ):'),
...
const App = () => {
  ...
  const [screen, setScreen] = useState('clientList');
  ...
  const renderScreen = () => {
    switch (screen) {
      case 'clientForm':
        return React.createElement(ClientForm, { client: isEditing ? activeClient : null, onSave: handleSaveClient, onCancel: () => setScreen('clientList') });
      case 'clientDetail':
        return React.createElement(ClientDetailScreen, {
          client: activeClient,
      ...
      case 'testSelection':
        return React.createElement(TestSelectionScreen, { onSelectTest: (test) => { setActiveTest(test); setScreen('testRunner'); }, onBack: () => setScreen('clientDetail') });
      case 'testRunner':
        return React.createElement(TestRunnerScreen, { client: activeClient, test: activeTest, onFinish: handleFinishTest, onBack: () => setScreen('testSelection') });
      case 'result':
        return React.createElement(ResultScreen, { result: activeResult, client: activeClient, onBack: () => setScreen('clientDetail'), onUpdateResult: handleUpdateResult, apiKey: apiKey });
      case 'clientList':
      default:
        return React.createElement(ClientListScreen, {
          clients: clients,
  ...
  if (!isInitialized) {
    return React.createElement('div', { className: 'card', style: { textAlign: 'center', padding: '40px' } }, 'Загрузка приложения...');
  }

  if (showKeyModal) {
    return React.createElement(ApiKeyModal, { onSave: handleSaveApiKey });
  }

  return React.createElement('div', null,
    renderScreen(),
    React.createElement('footer', { style: { textAlign: 'center', color: 'var(--secondary-text-color)', marginTop: '30px', padding: '10px' } },
      React.createElement('p', null, React.createElement('strong', null, 'Внимание!'), ' Все данные хранятся локально. Регулярно делайте резервные копии.'),
      React.createElement('div', { style: { display: 'flex', gap: '10px', justifyContent: 'center', marginTop: '10px' } },
        React.createElement('button', { className: 'secondary', onClick: handleExportData }, 'Экспорт данных'),
        React.createElement('button', { className: 'secondary', onClick: () => fileInputRef.current?.click() }, 'Импорт данных'),
        React.createElement('input', { type: 'file', ref: fileInputRef, onChange: handleImportData, style: { display: 'none' }, accept: '.json' })
      )
    )
  );
};
